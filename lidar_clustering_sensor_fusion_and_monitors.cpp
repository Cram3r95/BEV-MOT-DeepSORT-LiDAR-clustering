...
void sensor_fusion_and_monitors_cb(const sensor_msgs::PointCloud2::ConstPtr& lidar_msg, const t4ac_msgs::BEV_trackers_list::ConstPtr& bev_trackers_list_msg, const nav_msgs::Odometry::ConstPtr& ego_vehicle_pose_msg)
{
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr pedestrians_cloud (new pcl::PointCloud<pcl::PointXYZ>);
    pcl::PointCloud<pcl::PointXYZ>::Ptr vehicles_cloud (new pcl::PointCloud<pcl::PointXYZ>);
    ...
    *xyz_angle_filtered_cloud = aux_angle;
    ...
    int number_vehicles = 0, number_pedestrians = 0;

	*vehicles_cloud = aux_angle;
    cluster_filter(vehicles_cloud,1,8,400,&vehicles,&number_vehicles);

	*pedestrians_cloud = aux_angle;
    cluster_filter(pedestrians_cloud,1,3,50,&pedestrians,&number_pedestrians);
	...
	cout<<"\nOnly laser objects: "<<only_laser_objects.size();
    ...
    for (int i = 0; i < only_laser_objects.size(); i++)
	{
		...
		obstacle_points.header.stamp = lidar_msg->header.stamp;
		...
		obstacle_points.type = visualization_msgs::Marker::SPHERE;
	 	obstacle_points.id = only_laser_objects[i].object_id;
		...
		/*obstacle_points.pose.position.x = only_laser_objects[i].global_centroid_x;
		obstacle_points.pose.position.y = only_laser_objects[i].global_centroid_y;
		obstacle_points.pose.position.z = only_laser_objects[i].global_centroid_z;*/

        obstacle_points.pose.position.x = only_laser_objects[i].centroid_x;
		obstacle_points.pose.position.y = only_laser_objects[i].centroid_y;
		obstacle_points.pose.position.z = only_laser_objects[i].centroid_z;

		pub_LiDAR_Obstacles_Marker.publish(obstacle_points);
	}
